...
char *MotorController::get_sensor_type()
{
    Wire.beginTransmission(this->BASE_ADDRESS);
    Wire.write(SENSOR_TYPE);
    Wire.endTransmission(1);

    static char sensor_type[8];
    Wire.requestFrom(this->BASE_ADDRESS, 8);

    uint8_t i = 0;
    while (Wire.available())
    {
        sensor_type[i++] = Wire.read();
    }
    sensor_type[i] = '\0';
    return sensor_type;
}

char *MotorController::get_version()
{
    Wire.beginTransmission(this->BASE_ADDRESS);
    Wire.write(VERSION);
    Wire.endTransmission(1);
